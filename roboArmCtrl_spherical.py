...
def tranlateR(position,direction):
...
def imageFunction(function):
	...
	while math.sqrt(x**2 + y**2) > 10:
		...
		if(m['m00'] != 0.0): # moment is detected
			...
			x = int(m['m10']/m['m00'])
			y = int(m['m01']/m['m00'])
			errorX = calcError(cx, x)
			errorY = calcError(cy, y)
			
			if errorX > 0:
				servo.yaw = servo.yaw + 0.05
			else:
				servo.yaw = servo.yaw - 0.05
			position = FK(servo.yaw, servo.pitch, servo.elbow)
...
#servo.pitch = 0.5
#servo.elbow = 2
#current = FK(servo.yaw, servo.pitch, servo.elbow)
